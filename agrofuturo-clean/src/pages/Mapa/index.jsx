...
import { MapPin, Layers, Tractor, AlertTriangle, CheckCircle, Clock } from 'lucide-react'
import { api } from '../../services/api'
import { Card, SectionHeader, Badge, ProgressBar } from '../../components/ui'

const statusConfig = {
  pulverizando: { color: 'var(--accent-green)', label: 'Pulverizando', icon: Tractor },
  concluido: { color: 'var(--accent-blue)', label: 'Concluído', icon: CheckCircle },
  aguardando: { color: 'var(--text-muted)', label: 'Aguardando', icon: Clock },
  alerta: { color: 'var(--accent-yellow)', label: 'Alerta', icon: AlertTriangle },
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="animate-fadeIn">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {Object.entries(statusConfig).map(([key, conf]) => {
          const count = mapaField.talhoes.filter(t => t.status === key).length
          return (
            <Card key={key} style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{
              ...
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, color: conf.color }}>{count}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
        ...
        <Card style={{ padding: 24 }}>
          ...
          {/* Legend */}
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-light)' }}>
            {Object.entries(statusConfig).map(([key, conf]) => (
              <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--text-muted)' }}>
                <div style={{ width: 10, height: 10, borderRadius: 2, background: conf.color, opacity: 0.7 }} />
                {conf.label}
              </div>
            ))}
          </div>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Badge
                    variant={
                      t.status === 'concluido' ? 'info' :
                      t.status === 'pulverizando' ? 'success' :
                      t.status === 'alerta' ? 'warning' : 'default'
                    }
                    label={sc.label}
                  />
                </div>
                ...
                {t.status !== 'aguardando' && (
                  <ProgressBar value={t.progresso} color={sc.color} height={4} />
                )}
                {t.status !== 'aguardando' && (
