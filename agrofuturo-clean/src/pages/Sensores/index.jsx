...
import { Cpu, CheckCircle, AlertTriangle, Thermometer, Droplets, Wind, Battery, Activity } from 'lucide-react'
    ...
    <Card style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>{sensor.nome}</div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
          ...
          <Badge variant={isAlerta ? 'warning' : 'success'} label={isAlerta ? 'Alerta' : 'Ativo'} />
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {[
          { icon: Thermometer, label: 'Temp', value: `${sensor.temperatura}°C`, color: 'var(--accent-orange)' },
          { icon: Droplets, label: 'Umidade', value: `${sensor.umidade}%`, color: 'var(--accent-blue)' },
          { icon: Wind, label: 'Pressão', value: `${sensor.pressao} bar`, color: 'var(--accent-green)' },
          { icon: Activity, label: 'Fluxo', value: `${sensor.fluxo} L/m`, color: 'var(--accent-yellow)' },
      ...
      <div style={{
        ...
        marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border-light)',
      }}>
        ...
        <Badge variant={sensor.posicao === 'Esquerdo' ? 'info' : 'default'} label={sensor.posicao} />
      </div>
    </Card>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="animate-fadeIn">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Total de Sensores', value: sensores.length, color: 'var(--text-primary)' },
          { label: 'Operando Normal', value: ativos, color: 'var(--accent-green)' },
          { label: 'Em Alerta', value: alertas, color: 'var(--accent-yellow)' },
          { label: 'Cobertura', value: `${((ativos / sensores.length) * 100).toFixed(0)}%`, color: 'var(--accent-blue)' },
        ].map((item, i) => (
      ...
      <Card style={{ padding: 20 }}>
        <SectionHeader title="Diagrama da Pulverizadora" subtitle="John Deere R4045 — distribuição dos sensores nos braços" />
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center' }}>
            {sensores.filter(s => s.posicao === 'Esquerdo').map(s => (
              <div key={s.id} title={s.id} style={{
                width: 32, height: 32, borderRadius: 6,
                background: s.status === 'alerta' ? 'rgba(212,160,23,0.15)' : 'rgba(79,191,79,0.12)',
                border: `1px solid ${s.status === 'alerta' ? 'var(--accent-yellow)' : 'var(--accent-green)'}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                cursor: 'pointer',
              }}>
                <Cpu size={14} color={s.status === 'alerta' ? 'var(--accent-yellow)' : 'var(--accent-green)'} />
              </div>
            ))}
          </div>
          ...
          {/* Braço Direito */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center' }}>
            {sensores.filter(s => s.posicao === 'Direito').map(s => (
              <div key={s.id} title={s.id} style={{
                width: 32, height: 32, borderRadius: 6,
                background: s.status === 'alerta' ? 'rgba(212,160,23,0.15)' : 'rgba(79,191,79,0.12)',
                border: `1px solid ${s.status === 'alerta' ? 'var(--accent-yellow)' : 'var(--accent-green)'}`,
          ...
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            Braço Direito →
          </div>

          <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text-muted)' }}>
              <div style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--accent-green)', opacity: 0.6 }} />
              Sensor Ativo
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text-muted)' }}>
              <div style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--accent-yellow)', opacity: 0.6 }} />
              Sensor em Alerta
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setFiltro(t.key)} style={{
            ...
            background: filtro === t.key ? 'var(--accent-green)' : 'var(--bg-card)',
