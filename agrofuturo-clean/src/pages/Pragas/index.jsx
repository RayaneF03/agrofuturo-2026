...
import { Bug, MapPin, Calendar, CheckCircle, AlertTriangle, Filter } from 'lucide-react'
...
export default function Pragas() {
  ...
  useEffect(() => {
    api.getPragas().then(d => { setPragas(d); setSelected(d[0]) })
  }, [])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="animate-fadeIn">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Total Detectadas', value: pragas.length, color: 'var(--text-primary)' },
          { label: 'Não Tratadas', value: ativas, color: 'var(--accent-red)' },
          { label: 'Tratadas', value: tratadas, color: 'var(--accent-green)' },
          { label: 'Alta Severidade', value: pragas.filter(p => p.severidade === 'alta').length, color: 'var(--accent-yellow)' },
        ].map((item, i) => (
          <Card key={i} style={{ padding: '14px 18px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 700, color: item.color }}>
              {item.value}
            </div>
            ...
            return (
              <Card
                ...
                style={{
                  padding: '16px 18px',
                  cursor: 'pointer',
                  borderColor: isSelected ? 'var(--accent-green)' : p.tratada ? 'var(--border-light)' : sv.color + '40',
                  background: isSelected ? 'rgba(79,191,79,0.05)' : p.tratada ? 'var(--bg-card)' : `${sv.color}06`,
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{p.nome}</span>
                    ...
                    <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--text-muted)' }}>
                        <Calendar size={11} />
                        {new Date(p.detectadaEm).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div style={{ position: 'sticky', top: 24 }}>
            <Card style={{ padding: 22 }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 20 }}>
                <div style={{ fontSize: 56, lineHeight: 1, marginBottom: 12 }}>{selected.imagem}</div>
                ...
                <div style={{ marginTop: 10, display: 'flex', justifyContent: 'center', gap: 6 }}>
                  <Badge variant={severidadeConfig[selected.severidade].variant} label={`Severidade ${severidadeConfig[selected.severidade].label}`} />
                  {selected.tratada
              ...
              {[
                ...
                { label: 'Coordenadas', value: `${selected.coordenadas[0].toFixed(4)}, ${selected.coordenadas[1].toFixed(4)}` },
                { label: 'Detectada em', value: new Date(selected.detectadaEm).toLocaleString('pt-BR') },
                { label: 'Status', value: selected.tratada ? 'Área tratada' : 'Requer tratamento' },
              ].map((row, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', textAlign: 'right', maxWidth: 180 }}>
                    {row.value}
                  </span>
                </div>
              ))}
